...
while True:
    if flag == 1:
        ...
        b = re.findall(r'<dd><a target="_blank" href="http://www.mm131.com/xinggan/([0-9]*).html"><img src=', get.text)#获取点击页面
        for a in b:
            getpage = requests.get('http://www.mm131.com/xinggan/' + str(a) + '.html')
            tittle = re.findall(r'<h5>(.*)</h5>', str(getpage.content, 'gb2312', errors='ignore'))#获取头
            pages = []
            threads = []
            pages = re.findall(r'<span class="page-ch">共(.*?)页</span>', str(getpage.content, 'gb2312', errors='ignore'))#获取总页数
            page = pages[0]
            download_url = 'http://img1.mm131.me/pic/' + str(a) + '/'#图片下载地址（缺）
            for t in tittle:
        ...
        if get.status_code == 200:
            b = re.findall(r'<dd><a target="_blank" href="http://www.mm131.com/xinggan/([0-9]*).html"><img src=',
                           get.text)
            for a in b:
                getpage = requests.get('http://www.mm131.com/xinggan/' + str(a) + '.html')
